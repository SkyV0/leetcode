...
//    // the contents should be the key values (from the json) each on a separate line in the order they appeared in the json file (the file should end with a newline character on its own line). 
...
data.split(', ').forEach(e => {
    ...
    if(item[0] === 'age'){
     ...
     var item = e.split('=');
    if(item[0] === 'key'){
        return ages.push(parseInt(item[2]));
    }
});
console.log(age);
console.log(key);
