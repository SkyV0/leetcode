...
// let data = '' //save all the data in this variable
// resp.on('data', (chunk) => {
// data += chunk //all new lines are saved here
// })
// tempArray = [];
...
const hash = crypto.createHash('sha1');
https.get('https://coderbyte.com/api/challenges/json/age-counting', (resp) => {
    const count = resp.data(",").map(value=>value.split("=")).filter(value=> value[0]===" age").reduce((cumulative,current)=>{if(Number(current[1]; return cumulative+1;},0);
let data = '' //save all the data in this variable
resp.on('data', (chunk) => {
data += chunk //all new lines are saved here
})
